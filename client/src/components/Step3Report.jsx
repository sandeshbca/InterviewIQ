...
export default function Step3Report({ report }) {
  ...
  const downloadPDF = () => {
    ...
    if (finalScore >= 8) {
      advice =
        "Ready for job opportunities. Excellent clarity and structured responses.";
    } else if (finalScore >= 5) {
      advice =
        "Needs minor improvement before interviews. Good foundation, refine articulation.";
    } else {
      advice =
        "Significant improvement required. Work on clarity and confidence.";
    }

    doc.setFillColor(255, 255, 255);
    doc.setDrawColor(220);
    doc.roundedRect(margin, currentY, contentWidth, 20, 4, 4);
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-gray-50 to-green-50 px-4 sm:px-6 lg:px-10 py-8">
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button
        onClick={downloadPDF}
          className="bg-emerald-600 hover:bg-emerald-700 text-white py-3 p-4
        rounded-xl shadow-md transition-all duration-300 font-semibold text-sm sm:text-base text-nowrap"
        >
          Download PDF
        </button>
      </div>
